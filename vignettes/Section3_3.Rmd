...
## Distributional Instrumental Variables

`divR` implements Distributional Instrumental Variables (DIV). The target is the
interventional distribution $P(Y | do(X = x))$ when treatment is confounded and
an instrument is available.

2SLS focuses on a mean effect. Here the object is the whole distribution, so we
can also look at quantiles and tail behavior.
...
This example uses the softplus DGP from the paper. The function is nonlinear,
so it is a useful case where a linear IV fit is not the right benchmark.
...
The same hidden variable $H$ enters both equations, so $X$ and $Y$ are
correlated even apart from the causal effect of $X$.
